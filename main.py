...
def main():
    parser = argparse.ArgumentParser(
        description="ARAR Algorithm for H-Step Forecasting"
    )
    ...
    if not data_path.exists():
        ...
        y = load_data(
            data_path, config["data"]["date_column"], config["data"]["value_column"]
        )
    ...
    if config["analysis"]["run_arar"]:
        ...
        if config["model"]["arar"]["lag_selection_strategy"] == "powers_of_2":
            ...
            lags = select_reduced_lags(
                acf_vals,
                config["model"]["arar"]["max_lag"],
        ...
        forecast_index = pd.date_range(
            start=train.index[-1], periods=h + 1, freq=train.index.freq or "D"
        )[1:]
        y_forecast_arar_series = pd.Series(y_forecast_arar, index=forecast_index)
        plot_forecast_comparison(
            y, y_forecast_arar, forecast_index, h, output_dir / "arar_forecast_plot.png"
        )
        ...
        if config["analysis"]["run_arima_comparison"]:
            arima_model = fit_arima_model(
                train, tuple(config["model"]["arima"]["order"])
            )
