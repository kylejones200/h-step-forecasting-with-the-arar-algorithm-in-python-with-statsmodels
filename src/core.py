...
def load_data(
    data_path: Path, date_column: str = "date", value_column: str = "values"
) -> pd.Series:
...
def select_reduced_lags(
    acf_vals: np.ndarray, max_lag: int = 20, strategy: str = "powers_of_2"
) -> List[int]:
...
def generate_arar_forecast(model, h: int, last_value: float) -> np.ndarray:
    ...
    future_forecast = model.predict(
        start=len(model.model.endog), end=len(model.model.endog) + h - 1
    )
...
def plot_arar_vs_arima(
    ...
    train: pd.Series,
    test: pd.Series,
    y_forecast_arar: pd.Series,
    y_forecast_arima: pd.Series,
    forecast_index: pd.DatetimeIndex,
    mape_arar: float,
    mape_arima: float,
    output_path: Path,
