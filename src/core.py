...
def load_data(data_path: Path, date_column: str='date', value_column: str='values') -> pd.Series:
...
def select_reduced_lags(acf_vals: np.ndarray, max_lag: int=20, strategy: str='powers_of_2') -> List[int]:
...
def generate_arar_forecast(model, h: int, last_value: float) -> np.ndarray:
    ...
    future_forecast = model.predict(start=len(model.model.endog), end=len(model.model.endog) + h - 1)
...
def plot_series_comparison(original: pd.Series, differenced: np.ndarray, output_path: Path, plot: bool=False):
    ...
    plt.suptitle('ARAR Algorithm: Original and Differenced Series', fontsize=12, y=0.98, color='0.2')
...
def plot_forecast_comparison(y: pd.Series, y_forecast_arar: np.ndarray, forecast_index: pd.DatetimeIndex, h: int, output_path: Path, plot: bool=False):
    ...
    ax.plot(forecast_index, y_forecast_arar, label=f'{h}-Step Forecast', color='#D4A574', linewidth=1.2, linestyle='--')
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
    plt.savefig(output_path, dpi=100, bbox_inches='tight')
...
def plot_arar_vs_arima(y: pd.Series, train: pd.Series, test: pd.Series, y_forecast_arar: pd.Series, y_forecast_arima: pd.Series, forecast_index: pd.DatetimeIndex, mape_arar: float, mape_arima: float, output_path: Path, plot: bool=False):
    ...
    ax.plot(forecast_index, y_forecast_arar, label='ARAR Forecast', color='#D4A574', linewidth=1.2, linestyle='--')
    ax.plot(forecast_index, y_forecast_arima, label='ARIMA Forecast', color='#8B6F9E', linewidth=1.2, linestyle=':')
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
    plt.savefig(output_path, dpi=100, bbox_inches='tight')
